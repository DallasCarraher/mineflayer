...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  function tickPhysics (now) {
    ...
    if (hasClientTickEndPacket) {
      // Found live-testing against a 26.3 server: ServerGamePacketListenerImpl
      // only clears its per-tick receivedPositionThisTick/receivedMovementThisTick
      // bookkeeping when it receives this serverbound tick_end packet
      // (handleClientTickEnd). Without ever sending it, that flag is set true by
      // the very first position-bearing move packet and never reset again - so
      // the *second* position/position_look packet sent in the whole connection,
      // no matter how much later, gets the client kicked with
      // "multiplayer.disconnect.invalid_player_movement". The real client sends
      // this once per client tick; mirror that here.
